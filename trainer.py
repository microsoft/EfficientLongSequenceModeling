...
class Trainer(object):
    ...
    def _build_optimizer(self):
        s4_hook_name = "_s4_optimizer"
        params = list(
            filter(
                lambda p: p.requires_grad and not hasattr(p, s4_hook_name),
                chain(self.model.parameters(), self.criterion.parameters()),
            )
        )
        s4_params = list(
            filter(
                lambda p: p.requires_grad and hasattr(p, s4_hook_name),
        ...
        # update optimizer to contain S4 parameters
        s4_optimizer_params = [getattr(p, s4_hook_name) for p in s4_params]
        s4_optimizer_params = [
            dict(s) for s in set(frozenset(temp.items()) for temp in s4_optimizer_params)
        ]  # unique dicts
        for elem in s4_optimizer_params:
            temp_params = [p for p in s4_params if getattr(p, s4_hook_name, None) == elem]
            self._optimizer.optimizer.add_param_group(
                {"params": temp_params, **elem}
            )
